...
Rank <- read_csv("~/GIT/InternationalSchool/resources/ranking.csv", col_types = cols(RANK = col_number()), locale = locale(encoding = "UTF-8"))
Univ <- read_csv("~/GIT/InternationalSchool/resources/Univ.csv", locale = locale(encoding = "UTF-8"))
...
Match <- function(ID){
  ...
  if (dim(KableUniv)[1] != 0){
    cat("\n**", KableRank$RANK, "위 ",KableRank$UNIV, "** / ", KableRank$COUNTRY, " / [대학교정보](", KableRank$LINK, ")\n", sep = "")
    print(kable(KableUniv))
    cat("\n---\n")
  } else {
...
# Function MatchNoNum
MatchNoNum <- function(ID){
  KableRank <- Rank %>% filter(grepl(ID, UNIV))
  KableUniv <- UnivFinal %>% arrange(desc(ADM)) %>% filter(grepl(ID, UNIV)) %>% select(-NAVERID, -ADM, -NOTE)
  if (dim(KableUniv)[1] != 0){
    cat("\n**", KableRank$UNIV, "** / [대학교정보](", KableRank$LINK, ")\n", sep = "")
    print(kable(KableUniv))
    cat("\n---\n")
  } else {
    #cat("\n### ", KableRank$RANK, "위 ",KableRank$UNIV, " (", KableRank$COUNTRY, ")\n", sep = "")
...
## HYPSMC Schools
```{r}
UNIVID = c("Harvard", "Yale", "Princeton", "Stanford University", "MIT", "Caltech")
cat(paste0("\n- HYPSMC = ",paste(unique(UNIVID), collapse = ", ")), "\n\n")
...
## Ivy League Schools
```{r}
UNIVID = c("Harvard", "Yale", "Princeton", "Columbia University", "Dartmouth", "University of Pennsylvania", "Brown University", "Cornell University")
cat(paste0("\n- Ivy leagues = ",paste(unique(UNIVID), collapse = ", ")), "\n\n")
for (i in 1:length(UNIVID)){
  MatchNoNum(UNIVID[i])
}
